        ...
        for addon in chain(subrepo.glob("*"), subrepo.glob("addons/*")):
            if (addon / "__manifest__.py").is_file() or (
                ...
                if subrepo.name == "odoo":
                    local_path = "${{workspaceFolder:{}}}/addons/{}/".format(
                        subrepo.name, addon.name,
                    )
                else:
                    local_path = "${{workspaceFolder:{}}}/{}".format(
                        subrepo.name, addon.name
                    )
                debugpy_configuration["pathMappings"].append(
                    {
                ...
                path = "${{workspaceFolder:{}}}/{}/static/".format(
    ...
    cw_config["tasks"] = {
        ...
        "tasks": [
            ...
            },
            {
                ...
                "type": "process",
                "command": "invoke",
                "args": ["test", "--cur-file", "${file}", "--debugpy"],
                "presentation": {
                    "echo": True,
                    "reveal": "silent",
                    "focus": False,
...
def start(c, detach=True, debugpy=False):
    ...
    with tempfile.NamedTemporaryFile(
        ...
        if debugpy:
            ...
            _remove_auto_reload(
                tmp_docker_compose_file, orig_file=PROJECT_ROOT / "docker-compose.yml",
            )
        if detach:
            cmd += " --detach"
        with c.cd(str(PROJECT_ROOT)):
            result = c.run(
                cmd,
                pty=True,
                env=dict(UID_ENV, DOODBA_DEBUGPY_ENABLE=str(int(debugpy)),),
            )
            if not (
...
def _get_module_dependencies(
    ...
    if modules:
        cmd += f" -w {modules}"
    with c.cd(str(PROJECT_ROOT)):
        dependencies = c.run(cmd, env=UID_ENV, hide="stdout",).stdout.splitlines()[-1]
    return dependencies
...
def _test_in_debug_mode(c, odoo_command):
    with tempfile.NamedTemporaryFile(
        ...
        with c.cd(str(PROJECT_ROOT)):
            c.run(
                cmd, env=dict(UID_ENV, DOODBA_DEBUGPY_ENABLE="1",), pty=True,
            )
        _logger.info("Waiting for services to spin up...")
        time.sleep(SERVICES_WAIT_TIME)
...
def _get_module_list(
    ...
    with c.cd(str(PROJECT_ROOT)):
        module_list = c.run(
            cmd, env=UID_ENV, pty=True, hide="stdout",
        ).stdout.splitlines()[-1]
    return module_list
...
def test(
    ...
    for m_to_skip in skip.split(","):
        ...
        if m_to_skip not in modules:
            _logger.warn(
                "{} not found in the list of addons to test: {}".format(
                    m_to_skip, modules
                )
            )
...
def snapshot(
    c, source_db="devel", destination_db=None,
):
    ...
    if not destination_db:
        destination_db = "{}-{}".format(
            source_db, datetime.now().strftime("%Y_%m_%d-%H_%M"),
        )
    with c.cd(str(PROJECT_ROOT)):
        cur_state = c.run("docker-compose stop odoo db", pty=True).stdout
        _logger.info(
            "Snapshoting current {} DB to {}".format(source_db, destination_db)
        )
        _run = "docker-compose run --rm -l traefik.enable=false odoo"
        c.run(
...
def restore_snapshot(
    c, snapshot_name=None, destination_db="devel",
):
    ...
    with c.cd(str(PROJECT_ROOT)):
        ...
        _logger.info(
            "Restoring snapshot {} to {}".format(snapshot_name, destination_db)
        )
        _run = "docker-compose run --rm -l traefik.enable=false odoo"
        c.run(
